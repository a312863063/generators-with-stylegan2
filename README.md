...
&emsp;&emsp;最后，属性编辑器的制作原理是潜空间插值（这种方法适用于充分解耦的潜空间），简单来说就是构建一个标签二分类问题，求解出分隔平面后以其法向量（即系数w）作为近似的方向向量的方法求解。有趣的一点是，由于多款finetune生成器的差异主要(也不完全)是在G_mapping部分，而属性编辑器是针对G_synthesis操作的，所以这恰好可以解释为什么属性编辑器能适用于多款不同的生成器。<br/>
