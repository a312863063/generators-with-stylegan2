...
# 训练方法
&emsp;&emsp;此项目所有的模型都是是基于finetune制作出的，也就是用新的训练集去微调官方的模型。不过由于我的训练集的图片质量不如FFHQ数据集，因此训练难点在于如何在生成特定样式图片的同时保证图片清晰度。关键技巧在于，考虑到StyleGAN的人脸样式是由w空间（G_mapping）控制的，而骨干生成器（G_synthesis）决定人脸建模细节，因此当用新的数据集去finetune官方模型时，参数倾向于先快速缩放w空间的范围（即调整G_mapping中8-FC的参数）然后再调节G_synthesis的参数，这在训练过程中会表现为图片先快速产生样式的变化然后逐渐变得模糊（模糊是因为我的训练集不如官方的清晰），因此我们只需要卡在刚刚好样式调节完但还没有开始变模糊之前人为早停即可。<br/>
&emsp;&emsp;另外，由于多款finetune生成器的差异主要(也不完全)是在G_mapping部分，而属性编辑器是针对G_synthesis操作的，所以这可以解释为什么属性编辑器能适用于多款不同的生成器。对于混血脸生成器，它的本质是靠融合w空间（G_mapping）制作而成，也就是找到两个潜码分布的中间地带即可。最后，属性编辑器的制作原理是潜空间插值（这种方法适用于充分解耦的潜空间），简单来说就是构建一个标签二分类问题，求解出分隔平面后以其法向量（即系数w）作为近似的方向向量的方法求解。<br/>
