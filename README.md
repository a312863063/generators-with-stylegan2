...
&emsp;&emsp;另外，由于多款finetune生成器的差异主要(也不完全)是在G_mapping部分，而属性编辑器是针对G_synthesis操作的，所以这可以解释为什么属性编辑器能适用于多款不同的生成器。对于混血脸生成器，它的本质是靠融合w空间（G_mapping）制作而成，也就是找到两个潜码分布的中间地带即可。最后，属性编辑器的制作原理是潜空间插值，简单来说就是构建一个标签二分类问题，求解出分隔平面后以其法向量（即系数w）作为近似的方向向量的方法求解。<br/>
