...
&emsp;&emsp;In addition, since the differences between several finetuned generators are mainly (and not completely) in the G_mapping section, and the attribute editor is targeted at G_synthesis, this can explain why the attribute editor can be applied to multiple different generators. Finally, for the mixed-blood face generator, it is essentially made by merging w-space (G_mapping), that is, finding the middle zone of the two latent-code distributions.<br/>
